...
def callback(msg):
    ...
    desired_position.x = -0.25#msg.finish_line_position.pose.position.x
    desired_position.y = -0.16#msg.finish_line_position.pose.position.y
...
def fix_yaw(des_pos):
    ...
    print(f"Desired_yaw : {desired_yaw}")
    err_yaw = normalize_angle(desired_yaw - yaw)
    print(f'math.fabs(err_yaw): {math.fabs(err_yaw)}')
